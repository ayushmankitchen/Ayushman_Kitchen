...
export default function AdminDeliveryDispatcher() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setData(session);
      setDriver((current) => ({ name: session.driver_name || current.name, phone: session.driver_phone || current.phone }));
    } catch (error) {
  ...
  useEffect(() => {
    setLoading(true);
    load();
    return startVisiblePolling(load, data?.is_active ? 5000 : 15000);
  }, [load, data?.is_active]);
  ...
  const toggleGps = () => {
    ...
    watchRef.current = navigator.geolocation.watchPosition(async ({ coords }) => {
      ...
      try {
        ...
        setData((current) => current ? { ...current, current_lat: coords.latitude, current_lng: coords.longitude } : current);
      } catch (error) {
        stopGps();
        toast.error(apiError(error));
      }
    ...
    <section className="space-y-5" data-testid="delivery-dispatcher">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        ...
        <div className="flex rounded-2xl bg-white border border-stone-200 p-1 shadow-sm">
          {["lunch", "dinner"].map((slot) => <button key={slot} onClick={() => { stopGps(); setMealSlot(slot); }} className={`px-5 py-2.5 rounded-xl text-xs font-extrabold capitalize ${mealSlot === slot ? "bg-amber-400 text-slate-950" : "text-slate-600"}`}>{slot === "lunch" ? "☀️" : "🌙"} {slot}</button>)}
        </div>
      </div>
      ...
      <DeliveryMap driver={driverPoint} stops={mapStops} />
